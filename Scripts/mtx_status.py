...
# Edited: 14/4/2026
# The purpose of this script is to get a file with patient_id and mtx_success (0: fail, 1: success)
...
# ---- Select what patients we want to look at when determining mtx_success ---- #
# Print patients at start
count = df["patient_id"].nunique()
print("Number of patients before anything: ")
print(count)

### Ensure that patients do have a check-in visit 6 months after mtx_start ###
...
patients_no_follow_up = max_visit_dates[(max_visit_dates["Date"] - max_visit_dates["first_mtx_date"]) <= pd.Timedelta(days = 30.44*6)]
...
# ---- Determine if MTX discontinued in first year --- #
# Failure: Prescription stop of mtx lasts more than 3 months. Value of 0 in mtx_success
...
# Initialize dataframe with one row per patient to collect mtx_success
unique_ids = df["patient_id"].unique()
mtx_status = pd.DataFrame({"patient_id": unique_ids, "mtx_success": None})
...
for stops in mtx_events[mtx_events["Event"] == "Prescription_stop"].itertuples():
    ...
    # Else this stop indicates an mtx failure and is saved as such
        ...
        mtx_status.loc[mtx_status["patient_id"] == stops.patient_id, "mtx_success"] = 0

# Every value in mtx_success which is still none is a success and set to 1
mtx_status['mtx_success'] = mtx_status['mtx_success'].fillna(1).astype(int)


# ---- Save file ---- #
# Print info about it
print(f"Total patients: {len(mtx_status.index)}")
nan_included_counts = mtx_status['mtx_success'].value_counts(dropna=False)
