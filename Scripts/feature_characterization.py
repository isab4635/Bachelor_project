...
if label == "mtx_fail":
    ...
    df_features.drop(columns=["mtx_fail"], inplace=True)
else:
    raise ValueError("Label must be either 'mtx_fail' or 'biologic_added'")
...
for col in numerical_cols:
    ...
    # level alpha 2-sample test
    #test = stats.ttest_ind(subset_fail, subset_success, equal_var=False)
    #p_val = test.pvalue
    ...
    results_numerical.append({
        "Feature": col,
        "median_fail [IQR]": f"{median_fail:.2f} [{subset_fail.quantile(0.25):.2f} - {subset_fail.quantile(0.75):.2f}]",
        "median_success [IQR]": f"{median_success:.2f} [{subset_success.quantile(0.25):.2f} - {subset_success.quantile(0.75):.2f}]",
...
for col in categorical_cols:
    ...
    results_categorical.append({
        "Feature": col,
        "Fails (%)": f"{count_fail} ({pct_fail:.1f}%)",
        "Successes (%)": f"{count_success} ({pct_success:.1f}%)",
