...
# Create dataframe with each prediction
test_df = pd.DataFrame(
    {'True': y_test, 'Logistic': y_pred_logistic, 'RandomForest': y_pred_rf, 'DecisionTree': y_pred_dt, 'GradientBoost': y_pred_gbc})

fig, axs = plt.subplots(1, 2, figsize=(10, 5))


# Plot ROC with AUC
for model in ['Logistic', 'RandomForest', 'DecisionTree', 'GradientBoost']:
...
axs[1].legend()
plt.savefig(outdir + "boost_roc_pr.png")
...
for model in ['Logistic', 'RandomForest', 'DecisionTree', 'GradientBoost']:
    ...
    b = 0.5
    f_05 = (1 + b**2) * precision * recall / (b**2 * precision + recall + 1e-8)
    print(f"Precision: {precision:.2f}, Recall: {recall:.2f}, F1-score: {f1score:.2f}, F0.5: {f_05:.2f}")
